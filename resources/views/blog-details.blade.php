            ...
            <section class="py-14 md:py-[100px]">
                <div class="container">
                    <div class="pb-14 md:pb-20">
                        <h3 class="mb-7 text-xl font-extrabold text-black dark:text-white sm:text-2xl">{{ $blogs->title }}</h3>
                    ...
                    <div class="grid gap-[30px] sm:grid-cols-2">
                        <img src="{{ asset('storage/' . $blogs->thumbnail) }}" alt="{{ $blogs->title }}" class="rounded-[32px]" data-aos="fade-up" data-aos-duration="1000" />
                    </div>
                </div>
            </section>
                ...
                <div class="container">
                    <div class="mb-10 flex flex-col items-center justify-center gap-5 lg:mb-14 lg:flex-row lg:justify-between">
                        ...
                        <div class="flex items-center justify-end gap-4">
                            <a href="{{ route('blog.index') }}" class="text-sm font-extrabold text-black transition hover:text-secondary dark:text-white dark:hover:text-secondary">Semua Blog</a>
                        ...
                        @foreach ($relatedBlogs as $relatedBlog)
                            <div class="swiper-slide">
                                <div class="relative rounded-3xl border border-transparent bg-white transition duration-500 hover:border-secondary hover:bg-secondary/20 dark:bg-gray-dark">
                                    <a href="{{ route('blog.show', $relatedBlog->id) }}" class="absolute top-0 h-full w-full ltr:left-0 rtl:right-0"></a>
                                    <img src="{{ asset('storage/' . $relatedBlog->thumbnail) }}" alt="{{ $relatedBlog->title }}" class="h-52 w-full rounded-t-3xl object-cover" />
                                    <div class="p-5 text-sm font-bold">
                                        <h6 class="mb-1 text-black line-clamp-1 dark:text-white">{{ $relatedBlog->title }}</h6>
                                    </div>
                                </div>
                            </div>
                        @endforeach
